...
class ServerStats:
    ...
    async def main(self, bot, database, message, arguments):
        ...
        # Roles
        count_role, pos, srole = 0, 0, None
        for role in message.server.roles:
            count_role += 1
            if role.position > pos:
                pos, srole = role.position, role

        # Starting embed
        em=Embed(description="ID: "+message.server.id, colour=srole.colour)#0xf99e1a)
        ...
        # Query by
        em.set_footer(text="Query by: {}".format(message.author.name), icon_url=message.author.avatar_url)
